...
*predicted values is the mean of the boxplot in the terminal node
...
cit_grid_result %>%
  ...
  labs(title = "RMSE")
```
*each point is one of the hyperparameter algorithms that were tried

*RMSE - lower is better, thus, points at lower part (of y axis) is better
...
```
* higher better; at lower tree_depth value
...
final_fit %>%
  collect_predictions()
```
.pred = from taining; strength_gtex = test
...
* coz three terminal nodes

How can we get more horizontal lines in this type of algorithm?  

* spread data prediction; make data more liberal
...
In this exercise, we covered: 
  ...
  - Used **iterative search** to find the best values for mas_depth and min_criterion **used fixed grid search last time
